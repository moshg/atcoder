...
fn main() {
    ...
    r.read_line();
    let lens: Vec<i64> = r.read_line().split_whitespace().parse_all();
    // let lens: Vec<i64> = vec![2, 4, 3];
    // let lens: Vec<i64> = vec![100, 104, 102, 105, 103, 103, 101, 105, 104, 102, 104, 101];
    println!("{}", answer(&lens));
}

fn answer(lens: &[i64]) -> i64 {
    let total: i64 = lens.iter().map(|&len| len).sum();
    let half = total / 2;
    let mut before_half = 0;
    let mut after_half = 0;
    for &len in lens {
        let h = before_half + len;
        if h <= half {
            before_half = h;
            after_half = h;
        } else {
    ...
    let rest_before = total - before_half;
    let rest_after = total - after_half;

    let diff_before = (rest_before - before_half).abs();
    let diff_after = (rest_after - after_half).abs();
    cmp::min(diff_before, diff_after)
}
