...
fn main() {
    ...
    let (h, w, k): (isize, isize, isize) = r.read_line().split_whitespace().parse_all();
    let mut map = Vec::new();
    for _ in 0..h {
...
fn answer(map: &[Vec<bool>]) -> Vec<Vec<isize>> {
    let h = map.len();
    let w = map[0].len();
    let mut squares: Vec<Vec<isize>> = Vec::with_capacity(h);
    for _ in 0..h {
        squares.push(vec![0; w]);
    }
    ...
    for y in 0..h {
        for x in 0..w {
            ...
            let mut min_x = x;
            while min_x != 0 {
                if squares[y][min_x - 1] == 0 && !map[y][min_x - 1] {
                    min_x -= 1;
                } else {
                    break;
                }
            ...
            let mut max_x = x;
            while max_x != w - 1 {
                if squares[y][max_x + 1] == 0 && !map[y][max_x + 1] {
                    max_x += 1;
                } else {
                    break;
                }
            ...
            let mut min_y = y;
            'outer: while min_y != 0 {
                for nbr_x in min_x..(max_x + 1) {
                    if squares[min_y - 1][nbr_x] > 0 || map[min_y - 1][nbr_x] {
                        break 'outer;
                    }
                }
                min_y -= 1;
            ...
            let mut max_y = y;
            'outer: while max_y != h - 1 {
                for nbr_x in min_x..(max_x + 1) {
                    if squares[max_y + 1][nbr_x] > 0 || map[max_y + 1][nbr_x] {
                        break 'outer;
                    }
                }
                max_y += 1;
            ...
            for nbr_y in min_y..(max_y + 1) {
                for nbr_x in min_x..(max_x + 1) {
                    squares[nbr_y][nbr_x] = num;
                }
            }
            num += 1;
        }
