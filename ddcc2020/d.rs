...
fn main() {
    ...
    let m: usize = r.read_line().parse().unwrap();
    let mut digits: Vec<i64> = Vec::with_capacity(m);
    let mut counts: Vec<i64> = Vec::with_capacity(m);
    for _ in 0..m {
        let (d, c) = r.read_line().split_whitespace().parse_all();
        digits.push(d);
        counts.push(c);
    }
    // let digits = vec![1, 0, 7];
    // let counts = vec![1, 8, 1];

    let ans = answer(&digits, &counts, 0);
    println!("{}", ans);
}

fn answer(digits: &[i64], counts: &[i64], mut accum: i64) -> i64 {
    let mut sum = 0;
    ...
    if ones == 0 {
        ...
        let digits = vec![1, last];
        let counts = vec![ones, 1];
        answer(&digits, &counts, accum)
    }
}
