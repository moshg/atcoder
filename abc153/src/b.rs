...
fn main() {
    ...
    let (hp, _): (isize, isize) = r.read_line().split_whitespace().parse_all();
    let atks: Vec<isize> = r.read_line().split_whitespace().parse_all();
    let total: isize = atks.into_iter().sum();
    if total >= hp {
        println!("Yes");
    } else {
        println!("No");
