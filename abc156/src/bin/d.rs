...
fn main() {
    ...
    let (n, a, b): (u64, u64, u64) = r.read_line().split_whitespace().parse_all();
    let m = 1000000007;
    let mut total = modulo::pow(2, n, m);
    total = modulo::sub(total, modulo::comb(n, a, m), m);
    total = modulo::sub(total, modulo::comb(n, b, m), m);
    total -= 1;
    println!("{}", total);
}
