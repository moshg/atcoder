...
fn main() {
    ...
    let mut mans = Vec::with_capacity(n);
    for _ in 0..n {
        let man: (i64, i64) = r.read_line().split_whitespace().parse_all();
        mans.push(man);
    }

    let dirstr = r.read_line();
    let mut dirs = Vec::with_capacity(n);
    for c in dirstr.chars() {
        if c == 'R' {
            dirs.push(1);
        } else {
            dirs.push(-1);
    ...
    let true_mans: Vec<_> = mans
        .iter()
        .zip(dirs)
        .map(|(&(x, y), dir)| (x, y, dir))
        .collect();

    let collide = answer(&true_mans);
    println!("{}", if collide { "Yes" } else { "No" });
}

fn answer(mans: &[(i64, i64, i8)]) -> bool {
    let mut map = HashMap::with_capacity(mans.len());
    for &(x, y, dir) in mans {
        let v = map.entry(y).or_insert(Vec::new());
        v.push((x, dir));
    }

    for ymans in map.values() {
        let dirr = ymans
            .iter()
            .filter(|(_, dir)| *dir > 0)
            .map(|(x, _)| *x)
        ...
        if let Some(dirr) = dirr {
            if let Some(dirl) = dirl {
                if dirr < dirl {
                    return true;
                }
            }
