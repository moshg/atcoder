...
fn main() {
    ...
    r.read_line();
    let a: Vec<i64> = r.read_line().split_whitespace().parse_all();
    let b: Vec<i64> = r.read_line().split_whitespace().parse_all();

    let (x, y) = answer(&a, &b);
    println!("{}\n{}", x, y);
}

fn answer(a: &[i64], b: &[i64]) -> (i64, i64) {
    let perfect_sames = a.iter().zip(b).filter(|(x, y)| **x == **y).count();
    let aset: HashSet<i64> = a.iter().map(|x| *x).collect();
    let bset: HashSet<i64> = b.iter().map(|y| *y).collect();
    let sames = aset.intersection(&bset).count();
    (perfect_sames as i64, (sames - perfect_sames) as i64)
}
