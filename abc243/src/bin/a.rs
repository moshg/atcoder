...
fn main() {
    ...
    let input: Vec<i64> = r.read_line().split_whitespace().parse_all();
    let ans = answer([input[1], input[2], input[3]], input[0]);
    println!("{}", ans);
}

fn answer(amount: [i64; 3], rest: i64) -> char {
    let total = amount[0] + amount[1] + amount[2];
    let m = rest % total;
    if (m < amount[0]) {
        'F'
    } else if (m < amount[0] + amount[1]) {
        'M'
    } else {
        'T'
    }
