...
fn main() {
    ...
    let (_, mut _idx): (String, String) = r.read_line().split_whitespace().parse_all();
    let idx = BigInt::from_str(&_idx).unwrap();
    let mut idx_str = idx.to_str_radix(2);
    let moves: Vec<_> = r.read_line().chars().collect();
    answer(&mut idx_str, &moves);
    let idx = BigInt::from_str_radix(&idx_str, 2).unwrap();
    println!("{}", idx);
}

fn answer(idx: &mut String, moves: &[char]) {
    for &m in moves {
        if m == U {
            up(idx);
        } else if m == R {
...
fn up(idx: &mut String) {
    idx.pop();
}

fn right(idx: &mut String) {
    idx.push('1');
}

fn left(idx: &mut String) {
    idx.push('0');
}
