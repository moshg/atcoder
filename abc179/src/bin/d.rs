...
fn main() {
    ...
    let (n, k): (usize, usize) = r.read_line().split_whitespace().parse_all();
    let mut intervals: Vec<_> = Vec::with_capacity(k);
    for _ in 0..k {
        let interval = r.read_line().split_whitespace().parse_all();
        intervals.push(interval);
    }
    println!("{}", answer(n, &intervals));
}

const MOD: i64 = 998244353;

fn answer(n: usize, intervals: &[(usize, usize)]) -> i64 {
    let mut a = vec![0; n];
    a[0] = 1;
    a[1] = -1;
    [1, 1, 0, -1];
...
mod tests {
    ...
    #[test]
    fn test() {
        assert_eq!(answer(5, &[(1, 1), (3, 4)]), 4);
        assert_eq!(answer(4, &[(1, 2)]), 3);
    }
}
