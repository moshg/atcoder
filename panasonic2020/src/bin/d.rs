...
fn main() {
    ...
    let len: usize = r.read_line().parse().unwrap();
    let mut buf = String::new();
    let mut line = String::with_capacity(10);
    answer(len, &mut buf, &mut line, 0);
    print!("{}", &buf);
}

fn answer(len: usize, buf: &mut String, line: &mut String, unused: u8) {
    if line.len() == len {
        buf.push_str(&*line);
        buf.push('\n');
        return;
    ...
    for i in 0..unused {
        line.push((i + b'a') as char);
        answer(len, buf, line, unused);
        line.pop();
    }
    // N <= 26 なので分岐不要
    line.push((unused + b'a') as char);
    answer(len, buf, line, unused + 1);
    line.pop();
}
